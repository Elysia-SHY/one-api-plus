...
// FetchChannelUpstreamModels 实时拉取渠道上游 /models（不写目录），供渠道编辑页「从上游拉取」填入
func FetchChannelUpstreamModels(c *gin.Context) {
	id, err := strconv.Atoi(c.Param("id"))
	if err != nil || id <= 0 {
		c.JSON(http.StatusOK, gin.H{"success": false, "message": "无效的渠道 ID"})
		return
	...
	names, source, err := modelsync.FetchChannelModels(channel)
	if err != nil {
		c.JSON(http.StatusOK, gin.H{"success": false, "message": err.Error()})
		return
	}
