...
import (
	...
	"github.com/Elysia-SHY/one-api-plus/controller/auth"
...
func SetApiRouter(router *gin.Engine) {
	...
	apiRouter.Use(middleware.GlobalAPIRateLimit())
	{
		...
		// One API Plus：模型同步 / 健康检测 / 智能路由 / 别名 / 成本
		plusRoute := apiRouter.Group("/plus")
		{
			plusRoute.GET("/status", middleware.AdminAuth(), controller.GetPlusStatus)
			plusRoute.GET("/catalog", middleware.AdminAuth(), controller.GetModelCatalog)
			plusRoute.PUT("/catalog", middleware.AdminAuth(), controller.UpdateCatalogModel)
			...
			plusRoute.GET("/alias", middleware.AdminAuth(), controller.GetAliasList)
			plusRoute.POST("/alias", middleware.AdminAuth(), controller.AddAlias)
			plusRoute.PUT("/alias", middleware.AdminAuth(), controller.UpdateAlias)
			plusRoute.DELETE("/alias/:id", middleware.AdminAuth(), controller.DeleteAlias)
			plusRoute.GET("/price", middleware.AdminAuth(), controller.GetPriceList)
			plusRoute.POST("/price", middleware.AdminAuth(), controller.UpsertPrice)
			plusRoute.DELETE("/price/:id", middleware.AdminAuth(), controller.DeletePrice)
			plusRoute.GET("/cost", middleware.UserAuth(), controller.GetCostStat)
