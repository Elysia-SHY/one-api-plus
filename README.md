...
<p align="center">
  <a href="https://github.com/Elysia-SHY/one-api-plus"><img src="https://raw.githubusercontent.com/songquanpeng/one-api/main/web/default/public/logo.png" width="150" height="150" alt="one-api logo"></a>
</p>
...
_✨ 通过标准的 OpenAI API 格式访问所有的大模型，开箱即用 —— 小型 OpenRouter + Agent Gateway ✨_
...
<p align="center">
  ...
  <a href="https://github.com/Elysia-SHY/one-api-plus/releases/latest">
  ...
  <a href="https://goreportcard.com/report/github.com/Elysia-SHY/one-api-plus">
    <img src="https://goreportcard.com/badge/github.com/Elysia-SHY/one-api-plus" alt="GoReportCard">
  </a>
</p>

<p align="center">
  <a href="https://github.com/Elysia-SHY/one-api-plus#部署">部署教程</a>
  ·
  <a href="https://github.com/Elysia-SHY/one-api-plus#使用方法">使用方法</a>
  ·
  <a href="https://github.com/Elysia-SHY/one-api-plus/issues">意见反馈</a>
  ·
  <a href="https://github.com/Elysia-SHY/one-api-plus#截图展示">截图展示</a>
  ·
  <a href="https://openai.justsong.cn/">在线演示</a>
  ·
  <a href="https://github.com/Elysia-SHY/one-api-plus#常见问题">常见问题</a>
  ·
  <a href="https://github.com/Elysia-SHY/one-api-plus#相关项目">相关项目</a>
  ·
...
> 或者 [ghcr.io/songquanpeng/one-api](https://github.com/Elysia-SHY/one-api-plus/pkgs/container/one-api)
>
> alpha 版镜像地址：[justsong/one-api-alpha](https://hub.docker.com/repository/docker/justsong/one-api-alpha)
> 或者 [ghcr.io/songquanpeng/one-api-alpha](https://github.com/Elysia-SHY/one-api-plus/pkgs/container/one-api-alpha)
...
本项目在 [One API](https://github.com/songquanpeng/one-api) 的基础上做了一次「中转 → 网关」的升级，
把原来的单渠道转发改成了可自愈、可调度、可观测的小型 OpenRouter。

| 能力 | 说明 | 相关配置项 |
| --- | --- | --- |
| 模型自动同步 | 定时调用上游 `/models`，自动发现并入库新模型，支持 OpenAI / Azure / Gemini / Claude / DeepSeek / OpenRouter | `MODEL_SYNC_ENABLED`、`MODEL_SYNC_INTERVAL`、`MODEL_SYNC_AUTO_ENABLE` |
| 模型状态机 | 模型具备 `normal` / `degraded` / `paused` / `invalid` / `new` 五种状态 | `MODEL_SYNC_DEFAULT_STATUS` |
| 渠道健康检测 | 定时探测可用性与延迟，滑动平均统计错误率，异常渠道自动降权或暂停 | `HEALTH_CHECK_ENABLED`、`HEALTH_CHECK_INTERVAL`、`HEALTH_CHECK_AUTO_PAUSE` |
| 智能路由 | 同一模型绑定多渠道，按 `priority` / `weight` / `latency` / `cost` / `stability` 自动选路 | `ROUTING_STRATEGY` |
| 自动 Fallback | 429 / 超时 / 5xx / 配额不足时先换渠道，再按映射表换模型（如 GPT → Claude / DeepSeek） | `FALLBACK_ENABLED`、`FALLBACK_MODELS` |
| 模型别名 | 稳定短名映射到真实模型，方便 Agent / IDE 统一调用 | `ALIAS_ENABLED`、`DEFAULT_ALIASES` |
| 重复请求缓存 | 完全相同的非流式请求直接复用 Redis 缓存，不再消耗额度 | `RESPONSE_CACHE_ENABLED`、`RESPONSE_CACHE_TTL` |
| 成本与预算 | 模型定价、请求前成本预测、按日 / 按月预算控制、按模型费用统计 | `BUDGET_ENABLED`、`/api/plus/cost` |
| 安全增强 | 强制校验 API Key 的模型白名单（原版仅在 `/v1/models` 过滤） | 令牌「模型限制」 |
| Lite 模式 | 面向 OpenWrt / 随身 WiFi，关闭后台同步与检测任务降低内存占用 | `LITE_MODE=true` |

### 快速开启

```shell
# 智能路由按延迟优先，开启自动 fallback
-e ROUTING_STRATEGY=latency -e FALLBACK_ENABLED=true -e FALLBACK_MODELS='{"gpt-4o":["claude-3-5-sonnet-20241022","deepseek-chat"]}' # 每 6 小时同步一次上游模型并自动启用
-e MODEL_SYNC_ENABLED=true -e MODEL_SYNC_INTERVAL=360 -e MODEL_SYNC_AUTO_ENABLE=true # 每 5 分钟探测一次渠道健康度
-e HEALTH_CHECK_ENABLED=true -e HEALTH_CHECK_INTERVAL=5 # 模型别名（首次启动自动写入）
-e DEFAULT_ALIASES='{"gemini-flash":"gemini-2.5-flash-preview"}'
```

以上开关同样可以在管理后台「设置」里在线修改，无需重启。

### 新增接口

| 方法 | 路径 | 权限 | 说明 |
| --- | --- | --- | --- |
| GET | `/api/plus/status` | 管理员 | 增强特性运行状态总览 |
| GET | `/api/plus/catalog` | 管理员 | 模型目录（支持 `channel_id` / `status` / `keyword` 过滤） |
| PUT | `/api/plus/catalog` | 管理员 | 修改模型状态（正常 / 降级 / 暂停 / 失效） |
| POST | `/api/plus/catalog/enable` | 管理员 | 把目录中的模型启用到渠道 |
| POST | `/api/plus/sync` | 管理员 | 立即触发全量模型同步 |
| POST | `/api/plus/sync/:id` | 管理员 | 同步指定渠道 |
| GET | `/api/plus/health` | 管理员 | 全部渠道健康度 |
| POST | `/api/plus/health/check` | 管理员 | 立即触发健康检测 |
| GET | `/api/plus/health/channel/:id` | 管理员 | 单个渠道健康度 |
| GET/POST/PUT/DELETE | `/api/plus/alias[:/:id]` | 管理员 | 模型别名管理 |
| GET/POST/DELETE | `/api/plus/price[:/:id]` | 管理员 | 模型定价管理 |
| GET | `/api/plus/cost` | 用户 | 按模型聚合的费用统计（`days` 参数） |
...
如果启动失败，请添加 `--privileged=true`，具体参考 https://github.com/Elysia-SHY/one-api-plus/issues/482 。
...
### 手动部署
1. 从 [GitHub Releases](https://github.com/Elysia-SHY/one-api-plus/releases/latest) 下载可执行文件或者从源码编译：
   ```shell
   git clone https://github.com/Elysia-SHY/one-api-plus.git
...
详见 [#175](https://github.com/Elysia-SHY/one-api-plus/issues/175)。

如果部署后访问出现空白页面，详见 [#97](https://github.com/Elysia-SHY/one-api-plus/issues/97)。
